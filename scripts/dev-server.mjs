...
const staticRoot = process.argv[2] ? resolve(root, process.argv[2]) : root;
...
const server = createServer(async (request, response) => {
  ...
  const pathToServe = existsSync(filePath) ? filePath : join(staticRoot, 'index.html');
...
server.listen(port, '0.0.0.0', () => {
  console.log(`DESERTA website available at http://localhost:${port}`);
});
